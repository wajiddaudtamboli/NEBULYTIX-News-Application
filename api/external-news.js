...
const mapCategory = (gnewsCategory) => {
    const mapping = {
...
const transformGNewsData = (articles, category) => {
...
const fetchFromGNews = async (category, limit, country, fromDate, toDate) => {
    if (!GNEWS_API_KEY) {
...
const searchFromGNews = async (query, limit, fromDate, toDate) => {
    if (!GNEWS_API_KEY) {
...
export default async function handler(req, res) {
    ...
    try {
        const { category = 'general', limit = '10', type = 'news', country = 'in', from: fromDate, to: toDate, q: searchQuery } = req.query;
        ...
        if (type === 'search' || searchQuery) {
            ...
            const articles = await searchFromGNews(searchQuery, limitNum, fromStr, toStr);
        ...
        if (type === 'headlines') {
            const articles = await fetchFromGNews('general', limitNum, country, fromStr, toStr);
        ...
        if (type === 'featured') {
            const articles = await fetchFromGNews('general', 3, country);
        ...
        if (type === 'trending') {
            const articles = await fetchFromGNews('general', 10, country);
        ...
        const articles = await fetchFromGNews(categoryStr, limitNum, country, fromStr, toStr);
...
//# sourceMappingURL=external-news.js.map