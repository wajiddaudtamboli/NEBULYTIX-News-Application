...
// Get saved articles
async function handleSaved(req, res) {
    const clerkId = req.headers['x-clerk-user-id'];
    if (!clerkId) {
        return res.status(401).json({ success: false, error: 'Unauthorized' });
    }
...
// Save/unsave article
async function handleSaveToggle(req, res, newsId) {
    const clerkId = req.headers['x-clerk-user-id'];
    if (!clerkId) {
        return res.status(401).json({ success: false, error: 'Unauthorized' });
    }
    ...
    const isSaved = user.savedArticles.some((id) => id.equals(newsObjectId));
    if (isSaved) {
        user.savedArticles = user.savedArticles.filter((id) => !id.equals(newsObjectId));
    }
    else {
